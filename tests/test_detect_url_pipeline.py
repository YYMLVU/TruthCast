...
def test_detect_url_endpoint(mock_risk, mock_crawl):
    ...
    response = client.post("/detect/url", json={"url": "https://example.com/test"})
    ...
    assert data["image_urls"] == ["https://example.com/news.jpg"]
...
def test_detect_url_crawl_fail(mock_crawl):
    mock_crawl.return_value = MagicMock(
        success=False, image_urls=[], error_msg="404 Not Found"
    )

    response = client.post("/detect/url", json={"url": "https://example.com/fail"})
